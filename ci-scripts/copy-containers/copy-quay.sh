...
# Wallaby
copy-quay --token $TOKEN --from-namespace tripleowallaby \
...
copy-quay --token $TOKEN --from-namespace tripleorocky \
                    --to-namespace tripleorocky --job periodic-tripleo-centos-7-rocky-containers-build-push \
                    --html /root/logs/rocky-report.html copy &>>/root/logs/rocky.txt
