...
func getLatestGoodBuildURL(jobName string, opts *globalOptions) string {
	...
	if err != nil {
		logrus.Error("The HTTP request failed with error %v\n", err)
	} else {
		data, _ := ioutil.ReadAll(response.Body)
		if err := json.Unmarshal(data, &builds); err != nil {
			logrus.Errorln("The unmarshal failed with error %v", err)
		}
	}
...
func _fetch(URL string) (string, error) {
	response, err := http.Get(URL)
	if err != nil {
		logrus.Errorln("The HTTP request failed with error %v", err)
		return "", err
	}
