...
class PromoterAgent(object):
    ...
    # TODO maybe we can keep a smal text database with a list of all promotions
    # with timestampes.
    # it could be useful for tracking and metrics
    ...
    def validate_containers(self, dlrn_hash, name=None, assume_valid=False):
        results = {
            "hash_valid": False,
    ...
    def validate_hash(self, dlrn_hash, name=None, steps=['dlrn', 'qcows', 'containers']):
        ...
        # If we skip a test in validation, we assume that everything was done
        # without checking. We still call the functions, but they will just
        # crete "optimistic" responses
        # Check hash is present in dlrnapi
        validation_steps = ['dlrn', 'qcows', 'containers']
        validate_map = {
            'dlrn': self.dlrn_client.validate_hash,
            'qcows': self.qcow_client.validate_qcows,
            'containers': self.validate_containers,
        }

        for validation in validation_steps:
            assume_valid = False
            validator = validate_map[validation]
            if validation not in steps:
                assume_valid = True
            results[validation] = validator(dlrn_hash, name=name,
                                            assume_valid=assume_valid)
            valid = results[validation]['promotion_valid']
            promotion_validations.append(valid)
            valid = results[validation]['hash_valid']
    ...
    def analyze_statuses(self, hash_status):
        ...
        actions = {}
        container_actions = {}
        pprint.pprint(hash_status['containers']['registries'])
        for registry_name, status in hash_status['containers'][
                  'registries'].items():
    ...
    def rollback(self, promote_name, dlrn_hash, rollback_hash_status,
        ...
        if attempted_hash_status is not None:
            ...
            hash_status = copy.deepcopy(attempted_hash_status)
        else:
