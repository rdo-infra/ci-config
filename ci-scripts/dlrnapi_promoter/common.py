...
class DlrnHash(dict):
    ...
    def __init__(self, commit=None, distro=None, api_response=None):
        if commit is None and distro is None and api_response is None:
            self.commit_hash = ""
            self.distro_hash = ""
        if api_response is not None:
            self.commit_hash = api_response.commit_hash
            self.distro_hash = api_response.distro_hash
...
class DlrnClient(object):
    ...
    def fetch_hash(self, promote_name):
        ...
        print(api_response)
        try:
            return DlrnHash(api_response=api_response[0])
        except IndexError:
            return None
...
class QcowClient(object):
    ...
    def validate_qcows(self, dlrn_hash, name=None):
        ...
        try:
            self.client.stat(dlrn_hash.full_hash)
            results['hash_valid'] = True
            images = sorted(self.client.listdir(dlrn_hash.full_hash))
