    ...
    def validate(self, config, checks="all"):
        ...
        if "promotions" in checks:
            ...
            if promotions is None:
                ...
                validation_errors.append("Promotions section is empty")
            else:
                for target_name, info in promotions.items():
                    if 'criteria' not in info:
                        validation_errors.append("Missing criteria for target "
                                                 "{}".format(target_name))
                    if 'criteria' in info and not info['criteria']:
                        validation_errors.append("Empty criteria for target {}"
                                                 "".format(target_name))
                    if 'candidate_label' not in info:
                        validation_errors.append("Missing candidate label for "
                                                 "target {}"
                                                 "".format(target_name))
