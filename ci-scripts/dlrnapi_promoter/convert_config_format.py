...
def main(config_path_ini=None):
    ...
    config_promote_yaml = yaml.safe_dump(
        {'promote_from': dict(
            config_ini['promote_from'].items())}
    )
    config_tripleo_yaml = ""
    try:
        config_tripleo_yaml = yaml.safe_dump(
            {'current-tripleo': dict(
                config_ini['current-tripleo'].items())}
        )
    except KeyError:
        pass
    config_rdo_yaml = ""
    try:
        config_rdo_yaml = yaml.safe_dump(
            {'current-tripleo-rdo': dict(
                config_ini['current-tripleo-rdo'].items())}
        )
    except KeyError:
        pass
    config_staging_yaml = ""
    try:
        config_staging_yaml = yaml.safe_dump(
            {'tripleo-ci-staging-promoted': dict(
                config_ini['tripleo-ci-staging-promoted'].items())}
        )
    except KeyError:
        pass
