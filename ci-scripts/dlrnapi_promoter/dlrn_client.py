...
class DlrnClient(object):
    ...
    def __init__(self, config):
        ...
        self.osp_release = None
        self.distro_ver = None
        try:
            self.osp_release = config.osp_release
            self.distro_ver = config.distro_ver
        except AttributeError:
            self.log.info("This is not downstream promotion")
    ...
    def fetch_jobs(self, dlrn_hash):
        ...
        if hash_type is DlrnCommitDistroExtendedHash and (
                self.osp_release is not None and
                self.distro_ver is not None):
    ...
    def get_hash_from_component(self, log_header, component_name, base_url):
        ...
        if self.osp_release is not None and self.distro_ver is not None:
    ...
    def vote(self, dlrn_hash, job_id, job_url, vote):
        ...
        if type(dlrn_hash) == DlrnCommitDistroExtendedHash and (
                self.osp_release is not None and
                self.distro_ver is not None):
            dlrn_hash.dump_to_params(params)
