...
class DlrnClient(object):
    ...
    def fetch_jobs(self, dlrn_hash):
        ...
        if hash_type is DlrnCommitDistroExtendedHash and self.release.startswith('osp'):
    ...
    def get_civotes_info(self, dlrn_hash):
        ...
        hash_type = type(dlrn_hash)
        if hash_type is DlrnCommitDistroExtendedHashi and (
            self.release.startswith('osp'):
            civotes_info += (
