...
class DlrnClient(object):
    ...
    def get_promotion_aggregate_hashes(self, log_header, dlrn_hash,
        ...
        candidate_url = ""
        candidate_url1 = os.path.join(self.config.repo_url, commit_dir, "delorean.repo")
        candidate_url2 = os.path.join(self.config.repo_url, dlrn_hash.commit_dir,
                                      "delorean.repo")
        for r_path in [candidate_url1, candidate_url2]:
            try:
                if url.urlopen(r_path).code == 200:
                    candidate_url = r_path
                    break
            except (url.HTTPError, url.URLError):
                self.log.debug("Not valid url: {}".format(r_path))
