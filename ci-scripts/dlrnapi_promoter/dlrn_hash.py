...
class DlrnHashBase(object):
    ...
    def __init__(self, commit_hash=None, distro_hash=None, timestamp=None,
                 aggregate_hash=None, extended_hash=None, source=None,
        ...
        :param extended_hash: the computed extended part of the hash
        ...
        valid_attributes = {'commit_hash', 'distro_hash', 'aggregate_hash',
                            'extended_hash', 'timestamp', 'component'}
...
class DlrnExtendedHash(DlrnHashBase):
    """
    This class implements methods for the extended hash
    for the component pipeline
    It inherits from the base class and does not override the init
    """
    ...
    def sanity_check(self):
        ...
        """
        if self.commit_hash is None or self.distro_hash is None or \
                self.extended_hash is None:
            raise DlrnHashError("Invalid commit or distro or extended_hash")
    ...
    def __repr__(self):
        ...
        :return: The string representation of the object
        """
        return ("<DlrnExtendedHash object aggregate: %s, commit: %s,"
                " distro: %s, component: %s, timestamp: %s>"
                "" % (self.extended_hash, self.commit_hash, self.distro_hash,
                      self.component, self.timestamp))
    ...
    def __eq__(self, other):
        ...
        try:
            result = (self.extended_hash == other.extended_hash
                      and self.commit_hash == other.commit_hash
                      and self.distro_hash == other.distro_hash
                      and self.timestamp == other.timestamp)
        except AttributeError:
    ...
    def commit_dir(self):
        ...
        if self.label is not None:
            label_string = "{}/".format(self.label)
        commit_dir = "{}{}/{}/{}".format(label_string,
                                         self.extended_hash[:2],
                                         self.extended_hash[2:4],
                                         self.extended_hash)
        return commit_dir
...
class DlrnHash(object):
    ...
    Dlrn hash for the single pipeline or a Dlrn aggregate /Aggregate hash
    for the component pipeline It allows the DlrnHashBase class to be
    polymorphic, and work transparently for the caller as it does not
    have to worry if the hash is for the single or the integration
    pipelines
    """
    ...
    def __new__(cls, **kwargs):
        ...
        :param extended: the direct extended_hash
        ...
        :return: The DlrnCommitDistroHash or DlrnAggregateHash
        or DlrnExtendedHash instance
        """
        ...
        try:
            ...
            try:
                if kwargs['source']['aggregate_hash'] is not None:
                    hash_instance = DlrnAggregateHash(**kwargs)
                elif kwargs['source']['extended_hash'] is not None:
                    hash_instance = DlrnExtendedHash(**kwargs)
