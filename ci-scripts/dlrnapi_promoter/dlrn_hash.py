...
class DlrnCommitDistroExtendedHash(DlrnHashBase):
    ...
    def sanity_check(self):
        ...
        """
        if self.commit_hash is None or self.distro_hash is None or self.extended_hash is None:
    ...
    def __str__(self):
        ...
        """
        return ("commit: %s, distro: %s, extended: %s, component: %s, timestamp: %s"
                "" % (self.commit_hash, self.distro_hash, self.extended_hash,
                      self.component, self.timestamp))
    ...
    def full_hash(self):
        ...
        if self.extended_hash is not None:
            extend_distro_hash, extended_commit_hash = self.extended_hash.split('_')
            return '{0}_{1}_{2}_{3}'.format(self.commit_hash, self.distro_hash[:8], extend_distro_hash[:8], extend_commit_hash[:8])
        else:
