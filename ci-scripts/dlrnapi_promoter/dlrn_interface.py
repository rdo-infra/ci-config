...
class DlrnClient(object):
    ...
    def update_current_named_hashes(self, dlrn_hash, label):
        ...
        self.named_hashes_map.update({label: dlrn_hash.full_hash})
    ...
    def check_named_hashes_unchanged(self):
        ...
        for label, dlrn_hash in self.named_hashes_map.items():
            if latest_named_hashes[label] != dlrn_hash:
                self.log.error("Check named hashes: named hashes for label "
                               "'{}' changed since last check. At promotion "
                               "start: {}. Now: {}"
                               "".format(label, dlrn_hash,
                                         latest_named_hashes[label]))
    ...
    def fetch_jobs(self, dlrn_hash):
        ...
        try:
            ...
            self.log.error("------- -------- Promoter round aborted")
            raise
    ...
    def _promote_hash(self, dlrn_hash, target_label, candidate_label=None):
        ...
        promotion_hash_list = []
        if hash_type is DlrnAggregateHash:
            ...
            for component in components:
                baseurl = repo_config.get(component, 'baseurl')
                ...
                hash_info = {
                    'commit_hash': commits['commits'][0]['commit_hash'],
                    'distro_hash': commits['commits'][0]['distro_hash'],
                    'component': commits['commits'][0]['component'],
                    'timestamp': commits['commits'][0]['dt_commit']
                }
        ...
        elif hash_type is DlrnCommitDistroHash:
            self.log.debug("Dlrn promote '{}' from {} to {}: "
    ...
    def get_civotes_info(self, dlrn_hash):
        ...
        """
        if type(dlrn_hash) is DlrnCommitDistroHash:
        ...
        elif type(dlrn_hash) is DlrnAggregateHash:
            civotes_info = ("Check results at: "
                            "{}/api/civotes_agg_detail.html?"
                            "ref_hash={}"
                            "".format(self.config.api_url,
                                      dlrn_hash.aggregate_hassh))
