...
class DlrnClient(object):
    ...
    def _promote_hash(self, dlrn_hash, target_label, candidate_label=None):
        self.log.info("Promoting {} from {} to {}".format(dlrn_hash, candidate_label, target_label))
        promotion_hash_list = []
        if type(dlrn_hash) == DlrnAggregateHash:
            ...
            fd, tmprepo = tempfile.mkstemp()
            fp = open(tmprepo, "w+")
            with contextlib.closing(url.urlopen(candidate_url)) as repo_file:
                ...
                fp.write(repo_file.read().decode())
                fp.seek(0)
                repo_config.readfp(fp)
