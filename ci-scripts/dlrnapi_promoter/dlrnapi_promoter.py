...
# TODO(marios): remove this if we keep the include_tasks
def create_push_manifest(new_hashes, distro, release):
    ...
    commit_hash = new_hashes['commit_hash']
    try:
        logger.info('Creating + pushing manifest for dlrn hash %s on '
                ...
                try:
                    ...
                    if release not in ['ocata']:
                        tag_containers(
                            ...
                            release,
                            promote_name)
                        # create manifest with x86_64 & ppc64le containers
                        # TODO(marios) remove if we keep include_tasks
