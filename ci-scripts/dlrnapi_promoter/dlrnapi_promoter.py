...
class Log(object):
    ...
    def setup(self, targetlist, log_file=None):
        if targetlist is None:
            self.targetlist=["file"]
        else:
            self.targetlist = targetlist.split(",")
...
class Config(object):
    ...
    def __init__(self, logger):
        ...
        parser.add_argument("config_file", help="Release specific promotion config")
        parser.add_argument("--dry-run", help="Don't really promote anything", action="store_true")
        parser.add_argument("-f", help="Show logs in console", action="store_const", const="console")
        args = parser.parse_args()
        self.file_path = args.config_file
        self.parser = ConfigParser.SafeConfigParser(allow_no_value=True)
        self.logger = logger
        self.parser.read(args.config_file)
        self.log_file = self.parser.get('main', 'log_file')
        self.logger.setup(args.f, log_file=self.log_file)
        ...
        self.images_exclude_list = self.parser.get('main', 'images_exclude_list')
...
class Promoter(object):
    ...
    def __init__(self):
        ...
        # Set up env for external scripts.
        self.env = os.environ
        images_exclude = self.config.images_exclude_list.split(" ")
        args_list = ["-e "] * len(images_exclude)
 	    # this unreadable code just transform a string "a b c" into "-e a -e b -e c"
        self.env['RELEASE'] = self.config.release
        self.env['SCRIPT_ROOT'] = self.script_root
        self.env['DRY_RUN'] = str(self.config.dry_run)
    ...
    def tag_containers(self, candidate_hash, promote_name, images_list_url):
        self.env['IMAGE_EXCLUDE_LIST'] = " ".join(map( lambda x:" ".join(x), zip(args_list, images_exclude)))
        self.env['IMAGES_LIST_URL'] = images_list_url
        self.env['COMMIT_HASH'] = candidate_hash.commit_hash
        self.env['DISTRO_HASH'] = candidate_hash.distro_hash
        self.env['FULL_HASH'] = candidate_hash.full_hash
        self.env['PROMOTE_NAME'] = promote_name
        ...
        try:
            self.logger.info('Promoting the container images')
            output = subprocess.check_output(command, env=self.env,
    ...
    def tag_qcow_images(self, candidate_hash, promote_name):
        try:
            self.logger.info('Promoting the cloud images')
            command = ['bash']
...
#                raise PromotionSkip('candidate hash same as target')
        ...
        try:
            self.tag_containers(candidate_hash, target, images_list_url)
        ...
        try:
            ...
            self.logger.info('Hash promotion SUCCESSFUL')
        except ApiException:
            raise PromotionError
