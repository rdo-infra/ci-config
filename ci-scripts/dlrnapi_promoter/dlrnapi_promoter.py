    ...
    for promote_name, current_name in promote_from.items():
        ...
        for new_hashes in get_latest_hashes(api, promote_name, current_name,
            ...
            if missing_jobs:
                ...
                logger.info(
                    '%s/api/civotes_detail.html?commit_hash=%s&distro_hash=%s',
                    api_url,
            ...
            if dry_run:
                ...
                try:
                    ...
                    logger.info('SUCCESS \\0/ promoting %s %s as %s (%s)',
                                distro, current_name, promote_name, new_hashes)
                    logger.info(
                        '%s %s/api/civotes_detail.html?\
