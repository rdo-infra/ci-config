...
from dlrn_interface import DlrnHash, DlrnClient, DlrnClientError
...
class PromoterLogic(object):
        ...
        for selected_candidate in self.select_candidates(candidate_label,
            ...
            if not missing_jobs:
                try:
                    ...
                    self.log.info('FAILED promoting %s%s-%s %s as %s (%s)',
                                  self.config.distro, self.config.release,
                                  candidate_label, target_label,
                                  selected_candidate)
                    civotes_info = self.dlrnapi_client.get_civotes_info(
                        selected_candidate)
    ...
    def promote_all_links(self):
        ...
        for target_label, candidate_label in self.config.promotion_steps_map:
