...
class RepoClient(object):
    ...
    def get_containers_list(self, tripleo_common_commit, load_excludes=True):
        ...
        container_list = yaml.safe_load(containers_template.render())

        # parse 'container_images_template' key and fetch imagename
        # extract "<class 'jinja2.utils.Namespace'>/aodh-api:" to aodh-api
        # extract 'quay.io/tripleomaster/openstack-tempest:current-tripleo' to tempest
        if 'container_images_template' in container_list:
            full_list = [i['imagename'].rpartition('/')[-1].split(':')[0] for i in container_list['container_images_template']]
        elif 'container_images' in container_list:
            full_list = [i['imagename'].rpartition('/')[-1].split(':')[0] for i in container_list['container_images']]
