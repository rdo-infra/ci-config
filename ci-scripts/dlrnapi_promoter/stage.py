...
def parse_args(defaults, cmd_line=None):
    ...
    base_parser.add_argument('--scenes',
                             default=default_scenes,
                             help=("Select scenes to create in the"
                                   " environment scenes available {}"
                                   "".format(default_scenes))
                             ),
    base_parser.add_argument('--dry-run', action='store_true', default=False,
                             help="Don't do anything, still create"
                                  " stage-info")
    base_parser.add_argument('--promoter-user',
                             default=os.environ.get("USER", None),
                             help="The promoter user")
    base_parser.add_argument('--stage-info-file',
                             default=defaults['stage_info_file'],
                             help="Config file for stage results")
    base_parser.add_argument('--extra-settings',
                             help="Config file with additional settings")
    base_parser.add_argument('--db-data-file',
...
def main(cmd_line=None):
    ...
    log_file = config._constructor_log_file()
