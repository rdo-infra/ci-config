...
class TestPromoterConfigInstance(unittest.TestCase):
    ...
    def setUp(self):
        self.config = PromoterConfig()

    def test_base_instance(self):
        assert hasattr(self.config, "settings")
        self.assertIsInstance(self.config.settings, OrderedDict)
        self.assertIn("cli", self.config.settings)
        self.assertIn("file", self.config.settings)
        self.assertIn("default", self.config.settings)
        self.assertIn("experimental", self.config.settings)
        self.assertIsInstance(self.config.settings['cli'], dict)
        self.assertIsInstance(self.config.settings['file'], dict)
        self.assertIsInstance(self.config.settings['default'], dict)
        self.assertIsInstance(self.config.settings['experimental'], dict)
    ...
    def test_filter_allowed_clients(self):
        assert False

    def test_filter_distro_name(self):
        assert False

    def test_filter_promotions(self):
        assert False

    def test_constructor_api_url_(self):
        assert False
