...
from common import PromotionError, setup_logging
...
class DlrnSetup(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dlrn_hash_aggregate = DlrnAggregateHash(commit_hash='a',
                                                     ...
                                                     aggregate_hash='c',
                                                     timestamp=1)
        self.promote_log_header = ("Dlrn promote '%s' from %s to %s:",
...
class TestGetHashes(DlrnSetup):
    ...
    @patch('logging.Logger.error')
    def test_get_hash_from_component_success(self, mock_log_error):
        hash_info = {
            'dt_commit': 1,
            'timestamp': 1,
            'commit_hash': "a",
        ...
        tmp_dir = tempfile.mkdtemp()
        commits_yaml_path = os.path.join(tmp_dir, "commit.yaml")
    ...
    def test_promote_hash_inconsistent_response(self, api_promote_mock,
        ...
        mock_log_error.assert_has_calls([
            mock.call("%s (subhash %s) API returned different promoted hash:"
                      ...
                      'current-tripleo', self.dlrn_hash_commitdistro2)
        ])
        self.assertFalse(mock_log_info.called)
