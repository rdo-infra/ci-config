...
try:
    ...
    url_mock_string = 'urllib.request.'
...
class TestGetHashes(DlrnSetup):
    ...
    @patch(url_mock_string + 'urlopen')
    def test_get_hash_from_component(self, urlopen_mock):
        commits = [{
            'dt_commit': 1,
            'commit_hash': "a",
            'distro_hash': "b"
        ...
        __, commits_yaml_path = tempfile.mkstemp()
        with open(commits_yaml_path, "w") as commits_yaml:
            commits_yaml.write(yaml.dump(commits))
        base_url = "file:///{}".format(commits_yaml_path)
        # self.client.get_hash_from_component("", "component1", base_url)
