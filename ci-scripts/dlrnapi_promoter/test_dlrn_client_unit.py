...
class TestNamedHashes(DlrnSetup):
    ...
    def test_named_hashes_unchanged(self, mock_fetch_hashes):
        ...
        mock_fetch_hashes.side_effect = [self.dlrn_start_hash, self.dlrn_start_hash]
    ...
    @mock.patch('dlrn_client.DlrnClient.fetch_hashes')
    def test_check_named_hashes_changed(self, mock_fetch_hashes):
        mock_fetch_hashes.side_effect = [self.dlrn_start_hash, self.dlrn_changed_hash,
    ...
    @mock.patch('dlrn_client.DlrnClient.fetch_hashes')
    def test_fetch_current_named_hash(self, mock_fetch_hashes):
        self.client.fetch_current_named_hashes(store=True)
    ...
    @mock.patch('dlrn_client.DlrnClient.fetch_hashes')
    def test_update_current_named_hash(self, mock_fetch_hashes):
        mock_fetch_hashes.side_effect = [self.dlrn_changed_hash, self.dlrn_changed_hash]
        self.client.update_current_named_hashes(self.dlrn_changed_hash,
                                                "current-tripleo")
        self.client.fetch_current_named_hashes(store=True)
        self.client.check_named_hashes_unchanged()
