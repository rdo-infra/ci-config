...
class TestGetHashes(DlrnSetup):
    ...
    def test_get_promotion_commitdistro_hashes_success(self, mock_log_debug):
        promotion_list = self.client.get_promotion_commitdistro_hashes(
    ...
    def test_get_promotion_aggregate_hashes_success(self, mock_log_debug):
        promotion_list = self.client.get_promotion_aggregate_hashes(
...
class TestPromote(DlrnSetup):
    ...
    def test_promote_success(self, promote_hash_mock,
        ...
        mock_log_info.assert_has_calls([
            mock.call("Dlrn promote '%s' from %s to %s: moving previous "
                      "promoted hash '%s' to %s"
                      "", self.dlrn_hash_commitdistro1, 'tripleo-ci-testing',
                      'current-tripleo',
                      self.dlrn_hash_commitdistro2, 'previous-current-tripleo'),
            mock.call("Dlrn promote '%s' from %s to %s: Attempting "
                      "promotion", self.dlrn_hash_commitdistro1,
                      'tripleo-ci-testing',
                      'current-tripleo')
        ])
    ...
    def test_promote_no_previous(self, promote_hash_mock,
        ...
        mock_log_warning.assert_has_calls([
            mock.call("Dlrn promote '%s' from %s to %s: No previous promotion "
                      "found", self.dlrn_hash_commitdistro1,
