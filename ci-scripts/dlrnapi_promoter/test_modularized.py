...
from dlrn_interface import DlrnAggregatedHash, DlrnCommitDistroHash
...
different_commitdistro_notimestamp_kwargs = dict(commit_hash='a',
                                                 distro_hash='b')
valid_aggregated_kwargs = dict(aggregated_hash='a', timestamp=1)
valid_aggregated_notimestamp_kwargs = dict(aggregated_hash='a')
invalid_aggregated_kwargs = dict(aggregated='a')
different_aggregated_kwargs = dict(aggregated_hash='b',
                                   timestamp=1)
different_aggregated_notimestamp_kwargs = dict(aggregated_hash='a')
ambiguous_kwargs = dict(aggregated_hash='a', commit_hash='b', distro_hash='c')
# Structured way to organize test cases by hash type and source type
# by commitdistro and aggregated hash types and by dict or object source tyep
sources = {
    'commitdistro': {
        "dict": {
    ...
    'aggregated': {
        "dict": {
            "valid": valid_aggregated_kwargs,
            "valid_notimestamp":
                valid_aggregated_notimestamp_kwargs,
            'invalid': invalid_aggregated_kwargs,
            'different': different_aggregated_kwargs,
            'different_notimestamp':
                different_aggregated_notimestamp_kwargs
        },
        "object": {
            "valid": Mock(spec=type, **valid_aggregated_kwargs),
            "valid_notimestamp":
                Mock(spec=type, **valid_aggregated_notimestamp_kwargs),
            'invalid': Mock(spec=type, **invalid_aggregated_kwargs),
            'different': Mock(spec=type, **different_aggregated_kwargs),
            'different_notimestamp':
                Mock(spec=type, **different_aggregated_notimestamp_kwargs),
        },
    },
}
    ...
    def test_build_valid(self):
        for hash_type, source_types in sources.items():
            ...
            if hash_type == "commitdistro":
                dh = DlrnCommitDistroHash(values)
            ...
            elif hash_type == "aggregated":
                aggregated_hash = source_types['dict']['valid'][
                    'aggregated_hash']
                dh = DlrnAggregatedHash(values)
                self.assertEqual(dh.aggregated_hash, aggregated_hash)
        self.assertEqual(dh.timestamp,
                         source_types['dict']['valid']['timestamp'])

    def test_build_invalid(self):
        with self.assertRaises(DlrnHashError):
            DlrnCommitDistroHash(ambiguous_kwargs)
        with self.assertRaises(DlrnHashError):
            DlrnAggregatedHash(ambiguous_kwargs)


class TestDlrnHash(unittest.TestCase):

    def setUp(self):
        values = sources['aggregated']['dict']['valid']
        self.aggregated_hash_type = DlrnAggregatedHash(values).hash_type
        values = sources['commitdistro']['dict']['valid']
        self.commitdistro_hash_type = DlrnCommitDistroHash(values).hash_type

    def test_create_from_values(self):
        for hash_type, source_types in sources.items():
            dh = DlrnHash(**source_types['dict']['valid'])
            if hash_type == "commitdistro":
                self.assertEqual(dh.hash_type, self.commitdistro_hash_type)
            elif hash_type == "aggregated":
                self.assertEqual(dh.hash_type, self.aggregated_hash_type)

    def test_build_invalid(self):
        with self.assertRaises(DlrnHashError):
            DlrnHash(**ambiguous_kwargs)
        with self.assertRaises(DlrnHashError):
            DlrnHash(source=[])
    ...
    def test_properties(self):
        for hash_type, source_types in sources.items():
            ...
            if hash_type == "commitdistro":
                ...
                self.assertEqual(dh.full_hash, source.aggregated_hash)
    ...
    def test_dump_to_params(self):
        for hash_type, source_types in sources.items():
            ...
            if hash_type == "commitdistro":
                ...
                self.assertEqual(params.aggregated_hash, dh.aggregated_hash)
...
class TestDlrnClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        commitdistrohash_valid_attrs = ['commit_hash', 'distro_hash',
                                        'timestamp']
        aggregatedhash_valid_attrs = ['aggregated_hash', 'timestamp']
        self.api_hashes = []
        ...
        api_hashes_aggregated = []
        for idx in range(2):
            api_hash = Mock(spec=aggregatedhash_valid_attrs)
            api_hash.aggregated_hash = "a"
            api_hash.timestamp = 1
            api_hashes_aggregated.append(api_hash)
        self.api_hashes.append(api_hashes_aggregated)
        # Create an unordered list
        api_hashes_aggregated_unordered = []
        for idx in range(3):
            api_hash = Mock(spec=aggregatedhash_valid_attrs)
            api_hash.aggregated_hash = "a{}".format(idx)
            api_hash.timestamp = idx
            api_hashes_aggregated_unordered.append(api_hash)
        api_hash = api_hashes_aggregated_unordered.pop(0)
        api_hashes_aggregated_unordered.append(api_hash)
        self.api_hashes_unordered.append(api_hashes_aggregated_unordered)

    def tearDown(self):
        os.unlink(self.filepath)

    def test_hashes_to_hashes(self):
        # tests both commitdistro and aggregated
