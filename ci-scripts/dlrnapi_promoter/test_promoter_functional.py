...
def test_promote_containers(promoter, staged_env):
    ...
    # We need to check if the containers really promoted
    # assert False
...
def test_promote_qcows(promoter, staged_env):
    ...
    assert os.readlink(promotion_link) == promotion_dir, "Linked wrong dir"
    ...
    assert os.readlink(previous_link) == previous_dir, "No previous dir create"


@pytest.mark.parametrize("staged_env", ("all_single", "all_component"),
                         indirect=True)
