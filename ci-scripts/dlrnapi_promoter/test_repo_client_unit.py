...
class RepoSetup(unittest.TestCase):
    ...
    def setUp(self):
        ...
        tripleo_containers_list = ("quay.io/tripleomaster/openstack-base:current-tripleo\n"
                                   "quay.io/tripleomaster/openstack-os:current-tripleo\n"
                                   "quay.io/tripleomaster/openstack-aodh-base:current-tripleo\n")
...
class TestGetContainersList(RepoSetup):
    ...
    def test_get_containers_list_from_yaml_ok(self,
                                    mock_log_debug,
                                    mock_log_error):
        self.client.containers_list_path = 'tripleo_containers.yaml'
        containers_list = self.client.get_containers_list(
            self.versions_csv_rows[1]['Source Sha'])
        self.assertEqual(containers_list, ['base', 'os'])
        mock_log_debug.assert_has_calls([
            mock.call("Attempting Download of containers template at %s",
                      mock.ANY)
        ])
