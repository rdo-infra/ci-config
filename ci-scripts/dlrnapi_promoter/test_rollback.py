...
from transaction import RollbackNotNeeded
...
def test_rollback_basic(staged_env):
    ...
    h = pa.dlrn_client.fetch_hash(target_name)
    error_msg = "Unable to roll back the dlrn link"
    assert h == rollback_hash, error_msg
    ...
    assert res['promotion_valid'], "Hash was touched after rollback rejected"
