...
def test_promote_agent_validate_hash(staged_env, config):
    ...
    pa = rollback.PromoterAgent(config)
...
def test_promote_agent_promote_link(staged_env, monkeypatch, config):
    target_hash = ApiResponse()
    setattr(target_hash, "commit_hash", "a")
    setattr(target_hash, "distro_hash", "a")
...
def test_no_rollback_needed(staged_env, dlrn_offline, config):
