...
def parse_promotion_logs(test_config, stage_info):
    ...
    # we have a list of hashes at our disposal, we know which one will have to fail,
    # and which one will have to pass. We can do all in the same pass
    ...
    for commit in stage_info['commits']:
        if commit['full_hash'] == stage_info['promotions']['promotion_candidate']['full_hash']:
            # THis commit is suppoed succeed
        ...
        if commit['full_hash'] == stage_info['promotions']['failed_attempt']['full_hash']:
            # THis commit is suppoed to fail
