...
def check_dlrn_promoted_hash(test_config):
...
def query_container_registry_promotion(test_config)
    ''' Check that the hash containers have been pushed to the
        promotion registry with the promotion_target tag. '''

    registry_rdo = test_config['registry_rdo']
    registry_docker_io = test_config['registry_docker_io']
    promotion_target = test_config['promotion_target']
    commit_hash = test_config['commit_hash']
    distro_hash = test_config['distro_hash']
    # logger = logging.getLogger('TestPromoter')
...
def main():
    ...
    check_dlrn_promoted_hash(test_config)
    query_container_registry_promotion(test_config)
