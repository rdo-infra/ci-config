...
class StagedEnvironment(object):
    ...
    def setup_dlrn(self):
        ...
        check_cmd = ('pgrep -c -f "gunicorn.*dlrn.*"')
        try:
            subprocess.check_call(check_cmd.split())
        except subprocess.CalledProcessError:
            self.log.error("Cannot launch DLRN server: no gunicorn workers running")
    ...
    def teardown_dlrn(self):
        ...
        try:
            ...
            self.log.warning("Cannot shut down DLRN: no gunicorn process running")
        shutil.rmtree(root)
