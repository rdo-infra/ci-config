...
- A database usable by dlrnapi that already contains hashes, users, votes from jobs
...
- A hierarchy for the overcloud_containers_yaml file, to pass to the container-push playbook
  as a list of containers to promote
  see the overclooud_contaienrs_yaml subtree in sample/tree.txt
- a set of images to push to source registry, so the promoter has the container to pull and
  push during the promotion run
  see sample/docker_images.txt
- a meta.yaml file used to cleanup any local and remote configuration, including the local
  subtrees and local/remote containers.
  see sample/meta.yaml
...
class StagedHash(object):
    """
    This class orchestrates the various actions needed to emulate a promotion environment
    per hash. Each hash will need a set of images, containers, to prepare.
    FIXME: We could really setup a single hash, imagining it will be the only one the promoter logic
    will need to really handle.
    """
    ...
    def setup_images(self, meta):
        """
        For each hash, configure the images server. That is configure local paths
        for the sftp client to promote.
        The path created here mimics the hierarchy in the images server itself.
        It also injects a single empty image in the path. Probably not needed as the promoter cares only about
    ...
    def generate_yaml_file(self, images, meta):
        """
        The container-push playbook in the promoter gets a list of containers from a static
        position in a tripleo-common repo in a file called overcloud_containers.yaml.j2.
        We instead need to generate and pass to the promoter a file containing a limited subset of the
        containers present there, with arbitrary names.
        This function generates the relevant part of this file using the containers created, ready
        to be passed to the promoter logic as part of the staging test.
        """
        ...
        for distro, image_list in images.items():
            ...
            with open(container_file, 'w') as yf:
                for image_name in image_set:
                    # Tried to serialize this directly into yaml, but the dumper adds a \n after
                    # the name_suffix part for some strange reason
                    # The only part the container-push playbook is interested in is the one
                    # between the two prefixes, that's the only part that we are putting on the
                    # file
    ...
    def setup_repo_path(self):
        ...
        to an hash
        But AFAIU the dlrnapi server will not care if the directory is created or not,
        the promotion via dlrnapi just create a broken link to a directory that
        doesn't exist, but doesn't produce any error
        So unless we hit some error, we can completely ignore this part
        """
