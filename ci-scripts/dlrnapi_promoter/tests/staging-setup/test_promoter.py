...
def check_dlrn_promoted_hash(dlrn_host, promotion_target,
    ...
    try:
        ...
        raise
    try:
        assert any([(promotion.promote_name == promotion_target)
    ...
    if 'localhost' in registry_rdo:
        ...
        for line in lines:
            if full_hash in line:
                try:
                    line = line.replace(registry_rdo,
                        registry_docker_io)
                    source_image = docker_client.images.get(line)
                except docker.errors.ImageNotFound:
                    rg_data = docker_client.images.get_registry_data(
                        line)
...
def compare_tagged_image_hash(images_base_dir, user, key_path,
    ...
    if 'promoter-staging' not in images_base_dir:
        ...
        import pysftp
        with pysftp.Connection(host=images_base_dir,
            username=user, private_key=key_path) as sftp:
            try:
                images_dir = os.path.join('/var/www/html/images',
                    release, 'rdo_trunk')
                assert os.path.join(images_dir,
                    full_hash) == sftp.readlink(
                    os.path.join(images_dir, promotion_target))
            except AssertionError as e:
                print("Promotion target dir is not a
                    softlink of the full hash dir.")
                raise e
    else:
        # Check that the promotion_target dir is a soft link
        try:
            images_dir = os.path.join(images_base_dir,
                'overcloud_images', distro, release, 'rdo_trunk')
            assert os.path.join(images_dir,
...
def parse_promotion_logs(logfile, release, promotion_target,
    ...
    if status == 'success':
        ...
        for pattern in [success_pattern, success_pattern_images,
        ...
        fail_pattern = re.compile(
            r'promoter Skipping promotion of {\'timestamp\': (\d+), \'distro_hash\': \''  +
            re.escape(distro_hash) + r'\', (.*) \'full_hash\': \'' + re.escape(full_hash) +
            r'\', \'repo_hash\': \'' + re.escape(full_hash) + r'\', \'commit_hash\': \'' +
            re.escape(commit_hash) + r'\'}')
        try:
...
def main():
    ...
    check_dlrn_promoted_hash(test_config['dlrn_host'],
        test_config['promotion_target'],
        test_config['commit_hash'], test_config['distro_hash'])
    query_container_registry_promotion(test_config['registry_rdo'],
        test_config['registry_docker_io'],
        test_config['promotion_target'],
        test_config['commit_hash'],
        test_config['distro_hash'])
    compare_tagged_image_hash(test_config['images_base_dir'],
        test_config['user'],
        ...
        test_config['commit_hash'],
        test_config['distro_hash'])
    parse_promotion_logs(test_config['logfile'],
        test_config['release'],
        test_config['promotion_target'],
        test_config['commit_hash'],
        test_config['distro_hash'],
