...
def query_container_registry_promotion(registry_rdo, registry_docker_io, promotion_target, commit_hash, distro_hash):
    ...
    docker_client = docker.DockerClient(base_url=registry)
    # docker_client = docker.from_env()
    if 'localhost' in registry_rdo:
        base_path = os.path.dirname(os.path.abspath(__file__))
        images_promoted = open(os.path.join, base_path, '../../../',
                          'ci-scripts/dlrnapi_promoter/tests/staging-setup/samples/docker_images.txt'), 'r')
