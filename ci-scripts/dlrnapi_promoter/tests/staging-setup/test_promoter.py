...
def query_container_registry_promotion(registry_rdo, registry_docker_io,
                                       promotion_target, commit_hash, distro_hash):
    ...
    if 'localhost' in registry_rdo:
        ...
        images_promoted = open(os.path.join(
                               base_path, 'samples/docker_images.txt'), 'r')
        ...
        print("Compare images tagged with hash and those tagged promotion target:")
...
def parse_promotion_logs(logfile, release, promotion_target,
    ...
    if status == 'success':
        ...
        success_pattern_container = re.compile(
            r'promoter Promoting the container images for dlrn hash ' + re.escape(commit_hash))
        success_pattern_images = re.compile(
            r'Promoting the qcow image for dlrn hash ' + re.escape(full_hash) + r' on ' +
            re.escape(release) + r' to ' + re.escape(promotion_target))
        success_pattern = re.compile(
            r'promoter Successful jobs for {\'timestamp\': (\d+), \'distro_hash\': \''  +
