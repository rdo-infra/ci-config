...
baseurl=file://${distro_root}/${commit_dir}
'''
...
class StagingRepo(object):
    ...
    def __init__(self, config):
        ...
        self.repo_root_files = os.path.join(self.repo_root, self.distro_combo)

    def create_repo_hierarchy(self):
        """
    ...
    def create_commit_hierarchy(self, commit):
        ...
        abs_commit_dir = os.path.join(self.distro_root, rel_commit_dir)
...
class DlrnStagingServer(object):
    ...
    def promote_hash(self, commit, commit_dir):
        ...
        if not self.components_mode and promotion_alias is not None:
            link_path = os.path.join(self.distro_root,
    ...
    def setup(self):
        ...
        if not self.config.main['dry_run']:
            ...
            try:
                os.makedirs(self.distro_root)
            except OSError:
                self.log.info("Repo root dir exists, not creating")

        self.create_db()
        self.run_server()
        self.repo_stage_info = self.staging_repo.create_file_hierarchy()
    ...
    def stage_info(self):
        ...
        stage_info['server'] = {
            ...
            'repo_url': 'file://{}'.format(self.repo_stage_info),
