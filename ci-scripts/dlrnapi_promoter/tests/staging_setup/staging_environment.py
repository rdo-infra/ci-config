...
from dlrn_interface import DlrnCommitDistroHash
...
class StageConfig(object):
        ...
        if _config['main']['pipeline_type'] == "single":
            ...
            for promotion_alias, values in promotion_map.items():
                for index, db_commit in enumerate(db_commits[values['slice']]):
                    # The last commit (index=2) on each group, will be the last
                    # to promote and the promotion attached to that commit, will
                    # contain the correct aggregate hash we need to promote
                    if index == 2:
    ...
    def handle_overrides(self, _config, overrides):
        ...
        try:
            _config['dlrn']['server']['db_fixtures'] = overrides.fixtures_file
        except AttributeError:
            pass
...
def parse_args():
    ...
    parser.add_argument('--fixtures-file', default="scenario-1.yaml",
