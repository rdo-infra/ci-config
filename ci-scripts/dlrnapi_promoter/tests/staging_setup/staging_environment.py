...
try:
    ...
    from qcow_server import QcowStagingServer
    from registries import StagingRegistries
    from containers import StagingContainers
    from stage_config import StageConfig
except ImportError:
    # For when we are running the staging environemnt from the top directory
    from tests.staging_setup.dlrn_server import DlrnStagingServer
    from tests.staging_setup.qcow_server import QcowStagingServer
    from tests.staging_setup.registries import StagingRegistries
    from tests.staging_setup.containers import StagingContainers
    from tests.staging_setup.stage_config import StageConfig
...
class StagedEnvironment(object):
    ...
    def __init__(self, config):
        ...
        self.scenes_controllers = {
            ...
            "overcloud_images": QcowStagingServer(self.config),
            "containers": StagingContainers(self.config)
        }

        self.scenes = self.config.main['scenes']
        self.get_controllers(self.scenes)
    ...
    def get_controllers(self, scenes):
        self.controllers = {}
        for scene in scenes:
            try:
                self.controllers[scene] = self.scenes_controllers[scene]
            except KeyError:
                self.log.error("No controller found for scene %s", scene)
                raise

        if self.controllers == {}:
            raise Exception("No scene specifies")
    ...
    def setup(self):
        ...
        # We need dlrn stage info even if we are not running the setup
        stage_info_content['dlrn'] = self.scenes_controllers['dlrn'].stage_info
        for scene, controller in self.controllers.items():
            stage_info_content[scene] = controller.setup()
    ...
    def teardown(self):
        ...
        self.get_controllers(stage_info['main']['scenes'])
...
def parse_args(cmd_line=None):
    ...
    parser.add_argument('--fixtures-file',
                        default=defaults.fixtures_file,
    ...
    if hasattr(args, 'scenes'):
        args.scenes = tuple(args.scenes.split(','))
