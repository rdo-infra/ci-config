...
class PromoterTransaction(object):
    ...
    def rollback(self, attempted_hash=None, rollback_hash=None, name=None, retry_promotion=False):
        ...
        if attempted_hash is not None:
            ...
            # checkpoints
            attempted_hash_status = self.promoter_agent.validate_hash(attempted_hash, steps=self.incomplete_steps,
                                                        name=self.rollback_hash_name)
            if attempted_hash_status['promotion_valid']:
            ...
            if self.incomplete_steps == ['dlrn'] and attempted_hash_status['dlrn']['hash_valid']:
                self.log.info("Attempted hash {} is really just missing dlrn promotion", attempted_hash)
        ...
        rollback_hash_status = self.promoter_agent.validate_hash(rollback_hash)
        if rollback_hash_status['hash_valid'] is False:
            # TODO(gcerami): maybe we can think of a hash repair function
        ...
        #pprint.pprint(rollback_hash_status)
        #pprint.pprint(attempted_hash_status)
        self.promoter_agent.rollback(self.rollback_hash_name, rollback_hash,
                                     rollback_hash_status,
                                     attempted_hash_status)
