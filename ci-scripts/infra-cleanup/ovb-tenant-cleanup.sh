...
while [ "x$1" != "x" ]; do
    ...
    case "$1" in
        ...
        --dry-run|-d)
            ...
            STACK_ACTION="show"
            RESOURCE_ACTION_END=" -f json  |  jq -r '.[\"name\"] '"
            STACK_ACTION_END=" -f json  |  jq -r '.[\"stack_name\"] '"
            SLEEP_TIME=2
            ;;
...
for STACK in $STACK_LIST; do
    ...
    eval "openstack stack $STACK_ACTION $STACK $STACK_ACTION_END"
...
if [[  -z $STACK_LIST_STATUS ]]; then
    ...
    if [[ "$NUCLEAR" == "1" ]]; then
        ...
        for PORT in $PORT_LIST_EMPTY_ID; do
            ...
            eval "openstack port $RESOURCE_ACTION $PORT $RESOURCE_ACTION_END"
        done
    fi
    ...
    for STACK in $STACK_LIST_STATUS; do
        ...
        for SERVER in $SERVER_IDS; do
            ...
            eval "openstack server $RESOURCE_ACTION $SERVER $RESOURCE_ACTION_END"
        done
        ...
        for SUBNET_ID in $SUBNET_IDS; do
             ...
             for PORT_SUBNET_ID in $PORT_SUBNET_IDS; do
                 ...
                 eval "openstack port $RESOURCE_ACTION $PORT_SUBNET_ID $RESOURCE_ACTION_END"
             done
        done
        ...
        for NETWORK_ID in $NETWORK_IDS; do
            ...
            eval "openstack network $RESOURCE_ACTION $NETWORK_ID $RESOURCE_ACTION_END"
        done
