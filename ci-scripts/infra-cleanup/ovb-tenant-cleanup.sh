...
for STACK in $STACK_LIST; do
    ...
    openstack stack $STACK_ACTION $STACK
...
if [[  -z $STACK_LIST_STATUS ]]; then
    ...
    if [[ "$NUCLEAR" == "1" ]]; then
        ...
        for PORT in $PORT_LIST_EMPTY_ID; do
            ...
            openstack port $RESOURCE_ACTION $PORT
        done
    fi
    ...
    for STACK in $STACK_LIST_STATUS; do
        ...
        SERVER_IDS=$(openstack server list -f json |  jq -r --arg IDENTIFIER "$IDENTIFIER" '.[] | select(.["Name"] | contains($IDENTIFIER)) | .["ID"]')
        for SERVER in $SERVER_IDS; do
            echo "Deleting server ID $SERVER ..."
            openstack server $RESOURCE_ACTION $SERVER
        done

        # Delete ports with identifier in the name
        PORT_IDS=$(openstack port list -f json |  jq -r --arg IDENTIFIER "$IDENTIFIER" '.[] | select(.["Name"] | contains($IDENTIFIER))| .["ID"]')
        ...
        SUBNET_IDS=$(openstack subnet list -f json |  jq -r --arg IDENTIFIER "$IDENTIFIER" '.[] | select(.["Name"] | contains($IDENTIFIER)) | .["ID"]')
        for SUBNET_ID in $SUBNET_IDS; do
             ...
             for PORT_SUBNET_ID in $PORT_SUBNET_IDS; do
                 ...
                 openstack port $RESOURCE_ACTION $PORT_SUBNET_ID
             done
        done

        NETWORK_IDS=$(openstack network list -f json |  jq -r --arg IDENTIFIER "$IDENTIFIER" '.[] | select(.["Name"] | contains($IDENTIFIER)) | .["ID"]')
        for NETWORK_ID in $NETWORK_IDS; do
            echo "Deleting network ID $NETWORK_ID ..."
            openstack network $RESOURCE_ACTION $NETWORK_ID
        done
