...
def delete_individual_resources(stack_list, prefix, suffix, dry_run):
    ...
    """
    if stack_list == []:
        print("There are no stacks to delete")
    else:
...
def main(time_expired=300, dry_run=False, prefix="baremetal_", suffix=""):
    ...
    print(f"Stacks which was older than {time_expired} mins: {old_stack_list}")
    ...
    print(f"Stacks which are in  CREATE_FAILED or DELETE_FAILED state"
          f": {failed_stack_list}")
    ...
    print("Rechecking if there are stacks left in CREATE_FAILED"
          " or DELETE_FAILED state")
    failed_stack_list = failed_heat_stacks()
    print(f"Stacks which are in  FAILED state: {failed_stack_list}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t',
                        ...
                        'it will be deleted. It is used with the long-running'
                        ' option. Defaults to 300 minutes (5 hours)')
