...
class CloudResourcesTest(unittest.TestCase):
    ...
    mocked_router_list = [
        openstack.network.v2.router.Router(
            ...
            name='baremetal_73303-private_network-m77g7d4fwh4o-router-v4an5n2frvtf'),
        openstack.network.v2.router.Router(
    ...
    def test_env_accessibility_check_when_cloud_reachable(self, mock_conn, mock_connect):
        """ Positive test for env_accessibility_check function """
        mock_connect.return_value = mock_conn
        mock_conn.identity.get_token.return_value = 'gAA123'
        self.assertTrue(ovb_tenant_cleanup.env_accessibility_check())


    def test_env_accessibility_check_when_cloud_unreachable(self, mock_conn, mock_connect):
        """ Negative test for env_accessibility_check function """
        mock_connect.return_value = mock_conn
        mock_conn.identity.get_token.side_effect = Exception('Unable to reach cloud!')
        self.assertFalse(ovb_tenant_cleanup.env_accessibility_check())
    ...
    def test_servers_when_identifier_is_73303(self,  mock_conn, mock_connect):
        ...
        self.assertEqual(ovb_tenant_cleanup.servers_with_identifier("73303"),
    ...
    def test_servers_when_identifier_is_43666(self,  mock_conn, mock_connect):
        ...
        mock_connect.return_value = mock_conn
        mock_conn.compute.servers.return_value = iter(self.mocked_server_list)
        self.assertEqual(ovb_tenant_cleanup.servers_with_identifier("43666"),
                         ['b04d0ec1-5f95-480d-a1b2-e47d0929b706'])


    def test_servers_when_identifier_is_123456_negative_test(self,  mock_conn, mock_connect):
        """ Negative test for servers_with_identifier function """
        mock_connect.return_value = mock_conn
        mock_conn.compute.servers.return_value = iter(self.mocked_server_list)
        self.assertEqual(ovb_tenant_cleanup.servers_with_identifier("123456"), [])
    ...
    def test_subnet_when_identifier_is_73303(self, mock_conn, mock_connect):
        ...
        self.assertEqual(ovb_tenant_cleanup.subnets_with_identifier("73303"),
                         ['a1742089-430d-444e-a7ae-41f81270fac3'])

    def test_subnet_when_identifier_is_123456_negative_test(self, mock_conn, mock_connect):
        """ Negative test for subnet_with_identifier function """
        mock_connect.return_value = mock_conn
        mock_conn.network.subnets.return_value = iter(self.mocked_subnet_list)
        self.assertEqual(ovb_tenant_cleanup.subnets_with_identifier("123456"), [])

    def test_network_when_identifier_is_73303(self, mock_conn, mock_connect):
        """ Positive test for network_with_identifier function """
        mock_connect.return_value = mock_conn
        mock_conn.network.networks.return_value = iter(self.mocked_network_list)
        self.assertEqual(ovb_tenant_cleanup.networks_with_identifier("73303"),
                         ['448833ff-46bb-4689-841e-f965340af45d'])

    def test_network_when_identifier_is_123456_negative_test(self, mock_conn, mock_connect):
        """ Negative test for negative_with_identifier function """
        mock_connect.return_value = mock_conn
        mock_conn.network.networks.return_value = iter(self.mocked_network_list)
        self.assertEqual(ovb_tenant_cleanup.networks_with_identifier("123456"), [])

    def test_port_when_subnet_id_given(self, mock_conn, mock_connect):
        """ Positive test for ports_of_subnet function """
        mock_connect.return_value = mock_conn
        mock_conn.network.get_subnet_ports.return_value = iter(self.mocked_port_list)
    ...
    def test_port_when_subnet_id_is_none_negative_test(self, mock_conn, mock_connect):
        """ Negative test for ports_of_subnet function """
        mock_connect.return_value = mock_conn
        mock_conn.network.get_subnet_ports.return_value = iter(self.mocked_port_list)
        self.assertEqual(ovb_tenant_cleanup.ports_of_subnets(None), [])
    ...
    def test_router_when_identifier_is_73303(self, mock_conn, mock_connect):
        ...
        self.assertEqual(ovb_tenant_cleanup.routers_with_identifier("73303"),
            ['6bd7055f-8d5f-4656-b6b1-a1679faa1c91'])

    def test_router_when_identifier_is_123456_negative_test(self, mock_conn, mock_connect):
        """ Negative test for ports_of_subnet function """
        mock_connect.return_value = mock_conn
        mock_conn.network.routers.return_value = iter(self.mocked_router_list)
        self.assertEqual(ovb_tenant_cleanup.routers_with_identifier("123456"), [])

    def test_sec_gp_when_identifier_is_73303(self, mock_conn, mock_connect):
        """ Positive test for sec_gp_with_identifier function """
        mock_connect.return_value = mock_conn
        mock_conn.network.security_groups.return_value = iter(self.mocked_security_group_list)
        self.assertEqual(ovb_tenant_cleanup.sec_gp_with_identifier("73303"),
            ['1f5af4ed-e54d-4774-9299-3ac5fa2c5cd3'])

    def test_sec_gp_when_identifier_is_123456_negative_test(self, mock_conn, mock_connect):
        """ Negative test for sec_gp_with_identifier function """
        mock_connect.return_value = mock_conn
        mock_conn.network.security_groups.return_value = iter(self.mocked_security_group_list)
        self.assertEqual(ovb_tenant_cleanup.sec_gp_with_identifier("123456"), [])
...
class HeatTests(unittest.TestCase):
    mocked_heat_stacks = [
        openstack.orchestration.v1.stack.Stack(
            ...
            stack_owner='admin',
            parent=None,
            stack_user_project_id='55bd29e853694a379148bcb1b62ffbe9',
            tags=None),
        openstack.orchestration.v1.stack.Stack(
            ...
            stack_owner='admin',
            parent=None,
            stack_user_project_id='ecf6f14cc244437a92d1c6e628bd13fc',
            tags=None)]

    def setUp(self):
        self.mock_conn_patcher = mock.patch.object(openstack.connection,
                                                   'Connection', autospec=True)
        self.mock_connect_patcher = mock.patch('openstack.connect', autospec=True)
        ...
        self.mock_conn.orchestration.stacks.return_value = iter(self.mocked_heat_stacks)
    ...
    def test_old_heat_stacks_time_expired_5_hours(self):
        ...
        self.assertEqual(ovb_tenant_cleanup.old_heat_stacks(),
    ...
    def test_failed_heat_stacks(self):
        ...
        self.assertEqual(ovb_tenant_cleanup.failed_heat_stacks(), ['baremetal_73703'])
