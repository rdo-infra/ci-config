...
def get_rpm_logs(url):
    ...
    against two files.
    Error is neither a valid url or file
    location is passed.
    """
    ...
    try:
        ...
        if test_url.scheme in ['http' 'https']:
            ...
            last_lines = deque()
            nice_list = open(test_url.path, 'r', encoding='UTF-8').read().split()
        return nice_list
    except Exception as e:
        print("Passed neither file not url - " + url.replace('file://',''))
...
def diff_control_test(control_location, test_location, ignore_packages=None):
    ...
    nodes = {
                'undercloud': ["undercloud", "undercloud"]
            }
    ...
    package_diff = diff_build_fcns.diff_packages(control_list,
                                      test_list,
...
@click.command()
@click.option("--control_list", "-c", required=True, help="a url/file that points to list of rpms that are used as the control in the diff")
@click.option("--test_list", "-t", required=True, help="a url/file that points to the rpms to be compared against the control list")
@click.option("--ignore_list", "-i", required=False, help="a url/file that points to the rpms to be ignored in the comparison")
@click.option("--table_location", "-l", required=False, help="file location to write out diff table")
def main(control_list, test_list, ignore_list, table_location):
    ...
    diff_build_fcns.display_packages_table('repo_query',
                                 column_list,
