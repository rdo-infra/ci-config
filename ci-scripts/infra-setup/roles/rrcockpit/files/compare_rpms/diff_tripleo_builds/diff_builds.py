...
class DiffBuilds(object):
    ...
    def diff_packages(self, control_list, test_list, not_found_message="not installed"):
        """
    ...
    def get_the_nodes(self, cache, control_url, test_url, all_available, undercloud_only=False):
    ...
    def execute_installed_package_diff(self, cache, control_url, test_url, all_available, ignore_packages, undercloud_only):
        full_package_diff = {}
        logging.info(
        ...
        nodes = self.get_the_nodes(cache, control_url,
                                   test_url, all_available, undercloud_only)
        # TO-DO; include ceph
    ...
    def execute_repoquery_diff(self, cache, control_url, test_url, all_available, ignore_packages):
        full_package_diff = {}
        logging.info(
    ...
    def execute_compose_diff(self, cache, control_url, test_url, all_available, ignore_packages):
        full_package_diff = {}
        logging.info(
...
@ click.option("--undercloud_only", "-u", required=False, is_flag=True, default=False, help="use the undercloud only, ignore the controller, compute and ceph nodes")
def main(control_url, test_url, all_available, package_ignore, diff_compose, undercloud_only):
    """
