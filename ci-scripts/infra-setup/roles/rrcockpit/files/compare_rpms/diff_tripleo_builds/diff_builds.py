...
class DiffBuilds(object):
    ...
    def find_highest_version(self, packages):
        ...
        for key in packages:
            sanitized_list = {}
            for version in packages[key]:
                highest_version = version
                highest_version_calc = version
                try:
                    sanitized_list[version] = (int(version.replace('.', '')))
                    highest_version = max(sanitized_list.items(),
                                          key=operator.itemgetter(1))[0]
                    highest_version_calc = max(sanitized_list.items(),
                                               key=operator.itemgetter(1))[1]
                except:
                    logging.warning(
                        "can not convert {}-{} ".format(key, version))
                    continue
            dict_of_rpms[key] = [highest_version_calc, highest_version]
        return dict_of_rpms
...
def main(control_url, test_url, no_diff, all_available, package_ignore, diff_compose):
    ...
    if not all_available and not diff_compose:
        ...
        nodes = diff_builds.get_the_nodes(cached_sess, control_url,
                                          test_url, all_available)
        node_list = ["undercloud", "controller", "compute", "ceph"]

        for i in node_list:
            if i in nodes['control'].keys():
                control_list = diff_builds.get_logs(
                    cached_sess, control_url, nodes['control'][i])
                test_list = diff_builds.get_logs(
                    cached_sess, test_url, nodes['test'][i])
                control_list = diff_builds.parse_list(control_list)
                test_list = diff_builds.parse_list(test_list)

                control_list = diff_builds.find_highest_version(control_list)
                test_list = diff_builds.find_highest_version(test_list)

                package_diff = diff_builds.diff_packages(control_list,
                                                         test_list,
                                                         no_diff,
                                                         ignore_packages)
        ...
        nodes = diff_builds.get_the_nodes(cached_sess, control_url,
                                          test_url, all_available)
        control_list = diff_builds.get_repoquery_logs(cached_sess, control_url,
                                                      nodes['control']['undercloud'])
        test_list = diff_builds.get_repoquery_logs(cached_sess, test_url,
        ...
        #    def get_compose_logs(self, cache, base_url):
        control_list = diff_builds.get_compose_logs(cached_sess, control_url)
        test_list = diff_builds.get_compose_logs(cached_sess, test_url)
        # parse_compose(self, json_result):
        control_list = diff_builds.parse_compose(control_list)
        test_list = diff_builds.parse_compose(test_list)
        control_list = diff_builds.parse_list(control_list)
        test_list = diff_builds.parse_list(test_list)
