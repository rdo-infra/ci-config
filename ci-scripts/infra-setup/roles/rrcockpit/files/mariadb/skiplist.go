...
func main() {
    ...
    reportCSV(tests)
    // NOTE(arxcruz): Removing the logic behind csv and table, because go-pretty
    // table is having problem to compile in the old container, but keeping
    // here commented so we don't need to look again how to do it.
    // if is_csv {
    //     reportCSV(tests)
    // } 
    // else {
...
/* Commenting the reportTable function
