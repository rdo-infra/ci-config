...
def main():
    ...
    parser.add_argument("--log_file", default="", help="specifiy the file name")
    parser.add_argument(
        "--tempest_dump_dir",
        default=temp,
...
def get_last_build(
    job_name, tempest_log, tempest_dump_dir=tempfile.mkdtemp(prefix="skiplist")
):
    """
    ...
    if resp.status_code == 200:
        ...
        if cache.get(tempest_log_url) != 200:
            ...
            if resp_log.status_code == 200:
                tempest_file_name = download_tempest_file(
                    tempest_log_url, tempest_dump_dir
                )
                cache.add(
                    tempest_log_url, os.path.join(
                        tempest_dump_dir, tempest_file_name)
                )
...
def download_tempest_file(url, local_filename):
    ...
    local_filename: Result file (tempest.html, tempest.xml)
