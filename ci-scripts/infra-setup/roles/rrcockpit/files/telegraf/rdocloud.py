...
def run_server_check():
    ...
    d['undercloud'] = len([i for i in out
                           if i['Flavor Name'] == 'ci.m1.nodepool' and
                           "upstream-centos-7-rdo-cloud" in i['Name']])
    d['multinode'] = len([i for i in out
                          if "upstream-centos-7-2-node-rdo-cloud"
                          in i['Name']])
...
def run_quote_check():
    ...
    d = {}
    d['cores'] = next(iter(
        [i['Value'] for i in out if 'totalCoresUsed' in i['Name']]), 0)
    d['ram'] = next(iter(
        [i['Value'] for i in out if 'totalRAMUsed' in i['Name']]), 0)
    d['instances'] = next(iter(
        [i['Value'] for i in out if 'totalInstancesUsed' in i['Name']]), 0)
    d['gbs'] = next(iter(
        [i['Value'] for i in out if 'totalGigabytesUsed' in i['Name']]), 0)
    return d
...
def run_stacks_check():
    ...
    d['old_stacks'] = len([i for i in out
                           if int(
                              (datetime.datetime.now() -
                               datetime.datetime.strptime(
                               i['Creation Time'], '%Y-%m-%dT%H:%M:%SZ')
...
def write_influxdb_file(servers, quotes, stacks, ts):
    ...
    if servers:
        ...
        s += ('undercloud={undercloud},multinode={multinode},bmc={bmc},'
              'ovb-node={ovb-node},other={other},total={total},').format(
                **servers)
    if stacks:
        if not s:
            s = 'rdocloud-servers '
