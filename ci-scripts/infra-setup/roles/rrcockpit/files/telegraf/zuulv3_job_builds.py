...
def print_influx_ara_tasks(build, ara_json_file):
    ...
    try:
        ...
        for task in tasks:
            ...
            if duration > 0:
                print("build-task,name={},path={}"
                       " duration={} {}".format(
                          task['Name'].replace(' ', '\\ '),
                          ara_json_file,
                          duration,
                          to_ts(task['Time Start'], seconds=True, pattern=TIMESTAMP_PATTERN2)))
...
def print_influx(type, builds):
    if builds:
        for build in builds:
            build['type'] = type
            print_influx_ara_tasks(build, '/logs/ara.oooq.root.json')
            print_influx_ara_tasks(build, '/logs/ara.oooq.oc.json')
            print(influx(build))
