...
def get_mirror_info(mirror_file_name):
    ...
    mirror_key = os.path.basename(mirror_file_name).replace(".opendev.org.yaml",
                                                            ".main").replace('.', '-')
...
def get_all_mirrors(system_config_git_url, system_config_mirror_path):
    ...
    """
    mirrors = []
    mirror_dir = os.path.join(clone_repo(system_config_git_url), system_config_mirror_path)
    mirror_files = [os.path.join(mirror_dir, file) for file in os.listdir(mirror_dir) if file.startswith('mirror')]
