...
# Get dlrn md5 hash
def get_delorean_md5_hash(rdo_slug, rdo_source="https://trunk.rdoproject.org", dlrn_md5="delorean.repo.md5"):
    """
...
def get_rdo_proxy_url(mirror, distro, release, md5_hash):
    ...
    return posixpath.join(rdo_proxy_url, rdo_slug,
                          dlrn_md5_url, "delorean.repo.md5")
...
def run_sync(distro, release):
    ...
    for mirror in rdo_target_mirrors:
        ...
        print()
        print("{} -> {}".format(mirror,
                                verify_dlrn_md5(rdo_proxy_url,
                                                md5_sum)))

@click.command()
@click.option('--release', default='master',
              help='This is OpenStack Release')
@click.option('--distro', default='centos8',
              help='This is Distribution Name and version')
@click.option('--all', is_flag=True,
             help='Print mirror sync for all releases')
def main(distro, release, all):
    if all:
