...
def convert_builds_as_influxdb(queues):
    ...
    influxdb_line = "zuul-queue-status,url={url},pipeline={pipeline},queue={queue},job={name},review={review},patch_set={patch_set} result=\"{result}\",enqueue_time={enqueue_time},enqueued_time={enqueued_time},result_code={result_code}"
    epoch=int(time())
    for queue in queues:
