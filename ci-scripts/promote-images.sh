...
curl -L --silent --head --fail ${OPT_WEBSITE}/$BLEND/$RELEASE/rdo_trunk/$PROMOTED_HASH >/dev/null || {
    echo "ERROR: The source is invalid: ${OPT_WEBSITE}/$BLEND/$RELEASE/rdo_trunk/$PROMOTED_HASH"
    exit 3
}
...
curl -L --silent --head --fail ${OPT_WEBSITE}/$BLEND/$RELEASE/rdo_trunk/$LINK_NAME >/dev/null || {
    echo "ERROR: The target is invalid: ${OPT_WEBSITE}/$BLEND/$RELEASE/rdo_trunk/$LINK_NAME"
    exit 3
}
