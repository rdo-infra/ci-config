...
def convert_builds_as_influxdb(queue):
    ...
    influxdb_line = "zuul-queue-status,url={url},pipeline={pipeline},queue={queue},job={name},refspec={refspec} result=\"{result}\",enqueue_time={enqueue_time},enqueued_time={enqueued_time},result_code={result_code} {ts}"
    for refspec in queue['refspecs']:
        for job in refspec['jobs']:
            ...
            values['ts'] = format_ts_from_float(float(values['start_time']))
