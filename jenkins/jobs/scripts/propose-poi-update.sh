...
pushd ${PROJECT_DIR}
# repos.pp can be set up to use puppet-passed-ci (symlink) or an actual hash.
# Support both.
if grep -q 'puppet-passed-ci' manifests/repos.pp; then
    CURRENT_HASH="puppet-passed-ci"
else
