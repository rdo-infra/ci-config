...
echo ssh root@$VIRTHOST "cd $virthost_source_location && rsync -av $artifact_list $dest_centos_artifacts"

# push --> RDO file server (scp)
scp_opts="-v -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"
echo ssh root@$VIRTHOST "cd $virthost_source_location && scp $scp_opts $artifact_list $dest_rdo_filer"
