...
were hosted inside each role folder but this cases some problems:
...
keep using current practices, until we decide which approaches is better.
...
Prior to collections (2.9 or newer), there was no official way to install
Ansible extensions. Modules where supposed to included inside roles and visible
only to the role that had them.

Only collections allow you to install bundles of playbooks, roles, modules,
filters and plugins. Still, until our minimal supported version of Ansible
becomes 2.9, we cannot rely on them and we have to manually installed these.

It is in our interest to keep these Ansible components in the recomended
locations.

When Ansible runs under zuul, we can include **roles and modules** but **not
filters or plugins**. This is dues security measures as these run on Ansible
...
For example we learned this the hard way when working on log collection and
we had to rewrite a Jinja filter as an Ansible module in order to make it work
under zuul too.

Remember that Ansible can easily load JSON output produced by any script. This
means that you can easily write a simple `bash` or `python` script that
performs transformations that are very hard or impossible in pure ansible. If
that script is more then 3-4 lines of code is better to keep it separated in
order to be able to unit-test it using [pytest](https://docs.pytest.org/en/latest/)
for python or [bats](https://github.com/bats-core/bats-core) for bash.
